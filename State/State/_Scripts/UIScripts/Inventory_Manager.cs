...
public class Inventory_Manager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (showTooltip)
        {
            ...
            //显示Tooltip信息的位置
            Tooltip.GetComponentInChildren<UILabel>().transform.position = UICamera.currentCamera.ScreenToWorldPoint(Input.mousePosition);
        }
        else
        {
    ...
    void InitInventory()
    {
        for (int i = 0; i < count; i++)
        {
            ...
            Slot.transform.parent = GameObject.Find("inventory/Win/ScrollView/UIGrid").transform;
            Slot.transform.localScale = new Vector3(1, 1, 1);
            Slot.transform.parent.GetComponent<UIGrid>().repositionNow = true;
            //Slot.transform.parent.GetComponent<UIScrollView>().Press(true);
...
//         }
    ...
    public void SaveInventory()
    {
        for (int i = 0; i < inventory.Count; i++)
        {
            ...
            if (inventory[i].itemType == item.ItemType.Potion)
            {
                PlayerPrefs.SetInt("Inventory" + i, inventory[i].itemID);
                PlayerPrefs.SetInt("InventoryNum" + i, inventory[i].itemNum);
            }
    ...
    public void OnTidyClick()
    {
        for (int i = 0; i < inventory.Count; i++ )
        {
            ...
            {
                inventory[i] = PlayerPrefs.GetInt("Inventory" + i, -1) >= 0 ? datebase.items[PlayerPrefs.GetInt("Inventory" + i)] : new item();
                inventory[i].itemNum = PlayerPrefs.GetInt("inventoryNum" + i);
            }
            else if (inventory[i].itemType == item.ItemType.Material)
            {

            }
            else if (inventory[i].itemType == item.ItemType.TaskType)
            {
